...
dpath='/media/hdhs/data/eeg_propofol_data/';
load([dpath,'eeg_noinit/pam_eeg04_2018_6_12_18h53_6s_50ite.mat'])% 0
load([dpath,'pam_eeg04.mat'])% 
load([dpath,'eeg_noinit/pam_eeg04_bootstrap_500.mat'])% 2
load([dpath,'eeg_noinit/pam_eeg04_bootstrap_500_final.mat'])% 3
load([dpath,'rawEEg/eeganes04_laplac250_ch36.mat'])%4
...
for tt=1:T
    ...
    R_tot(1,tt)=R_tmp;
    [H_tot, H_i]=get_theoretical_psd(f_y,Fs,f_tot(:,tt)',a_tot(:,tt)',q_tot(:,tt));
    s_tot(:,tt)=H_tot';
...
%% Figure 2 : comparison parametric / standard PAM + Spectrum with behavioral test. 
DecimateFact=10; % Might be changed
ylmax=20;ylmin=0;
clmin=-15;clmax=20;
plotBOOT=1;
...
pProb=subplot(6,2,3);hold on;
pr=plot(BHVR.T'/(FsMAX*60),BHVR.prob_verbal.p500 ,'color','k','linewidth',2);
[fillhandle,msg]=jbfill(BHVR.T'/(FsMAX*60),BHVR.prob_verbal.p975',BHVR.prob_verbal.p025',[0.5 0.5 0.5]);
hold on;
%pb=plot(BHVR.T'/(FsMAX*60),BHVR.prob_burst.p500 ,'color','b','linewidth',2);
%[fillhandle2,msg2]=jbfill(BHVR.T'/(FsMAX*60),BHVR.prob_burst.p975',BHVR.prob_burst.p025','b');
%legend([pr,pb],{'Verbal','Click'})
...
colormap(p1, jet)
colormap(p2, jet)
pos1=get(subplot(3,2,3), 'Position');
pos2=get(subplot(3,2,4), 'Position');
highBar=pos1(4);
widtBar=highBar/20;
...
% K mod
p3=subplot(6,2,2); hold on
if plotBOOT
plot(DecimateFact*(1:length(K_mod_t_k(1,:)))/(60*Fs),K_mod_t_k(:,:), 'color', [0.5 0.5 0.5])
end
plot(DecimateFact*(1:length(K_mod_t_k(1,:)))/(60*Fs),K_mod_t_k(1,:), 'color', [0    1 0])
%title(' K^{mod}_t')
axis tight
set(gca, 'XTick', xTickTot);
box on
...
% Phi mod
p4=subplot(6,2,4); hold on
if plotBOOT
    for kk=1:Kappa+1
        scatter(DecimateFact*(1:length(K_mod_t_k(1,:)))/(60*Fs), Phi_mod_t_k(kk,:), 0.001, [0.5 0.5 0.5])
    end
end
scatter(DecimateFact*(1:length(K_mod_t_k(1,:)))/(60*Fs), Phi_mod_t_k(1,:), 1, [0 1 0])
axis tight
ylim([-pi pi])
%title('\phi^{mod}_t')
xlabel('(min)');
...
% Parametric PAM
Npb2=18;theta=linspace(-pi, pi,Npb2);
PAMequiv =  (1+2*sin(pi/Npb)*mean(K_mod_t_k(1,:),1).*cos(mean(Phi_mod_t_k(1,:),1)+theta')*Npb/(2*pi));
p6=subplot(3,2,6);
...
% PAM colorbar
pos1=get(subplot(3,2,5), 'Position');
pos2=get(subplot(3,2,6), 'Position');
middleFig=(pos1(1)+pos2(1)+pos2(3))/2-widtBar/2;
cax = colorbar('Position', [middleFig pos1(2)  widtBar/1.2 highBar]); %caxis([0.5 1.5]);
set(gca, 'clim', [0.2 1.8]); %0.2-1.8
...
% Modfy xaxis if necessary
p6=subplot(3,2,6);
xlim([0 150])

%Save 
h=gcf;
set(h,'Position',[411   196   918   898]);
set(h,'PaperOrientation','landscape');
print(gcf, '-dpdf', 'figure2subject04_300ite2.pdf')
...
pos2=get(subplot(4,2,4), 'Positionbox on');
...
set(gcf,'Position',[961           1         1280 1460]);
pathDrop='/home/nsrlhs/Dropbox (MIT)/producedFig/';
print(gcf, '-djpeg', [pathDrop,'decomposotionNonCenter300ite_grey'])
